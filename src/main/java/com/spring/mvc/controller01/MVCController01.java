...
@RequestMapping("/chap01") // 클래스 위에 리퀘스트 맵핑을 붙이게 되면 클래스내 모든 리퀘스트 맵핑에 자동으로 붙게됨
public class MVCController01 {
    ...
    // 배열로 메서드 2개 나열도 가능 2개이상 메서드 허용시 무조건 아래 방법을 해야함
    ...
    public String person(HttpServletRequest request){
        ...
        System.out.println("name파라미터 전송값 : " + age);
        return "";
    }

    /*
        2. @RequestParam 사용하기
         /major?stu=kim&major=computerscienc&gpa=3.4
         메서드 선언부에 파라미터와 자료형을 입력하고, 왼쪽에 @RequestParam("가져올변수명") 입력
         자료형을 자동으로 바꿔줌
     */
    @RequestMapping("/major")
    public String major(String stu, @RequestParam(defaultValue = "0.0") double gpa, // 파라미터 입력 안될시 기본값 지정도 가능
                        // @RequestParam에 선언된 파라미터값 변수는 프로그램상에서 사용되는 변수와 브라우저상의 변수를 다르게 할수 있음
    ...
    /*
        3. 커맨드 객체 이용하기
           연관된 자료를 한번에 처리해야 하는 경우 외부에 class를 하나 만들고 그 클래스를 파라미터로 주면
    ...
    public String car(Car car){
        System.out.println("파라미터로 전달 받은 Car 내부 : " + car);
        return "";
    }
    ...
    // /membor/회원이름/회원번호
    // 패턴으로 파라미터를 처리하도록 만들어 주세요
    // 회원 이름은 mName, 회원번호는 mNo 명칭을 사용합니다.
    @RequestMapping("/member/{mName}/{mNo}")
    public String board(@PathVariable String mName, @PathVariable long mNo){
    ...
    //carfrom 에서 보낸 데이터를 처리해주는 페이지 생성
    ...
    // chap01폴더 내에 loginform.jsp로 지정해 주세요
   ...
   @RequestMapping(value = "/login", method=RequestMethod.POST)//보낸 아이디와 비밀번호를 받아서 화면에 찍어주는 로직을 작성해 주세요
