...
public class MVCController03 {
    ...
    public String naver(){
        ...
        // 마치 a태그를 클릭하는것 처럼 자동으로 페이지를 보낼주소로 이동시킵니다.
        // 이 과정에서 포워딩과는 달리 model.addAttribute() 처럼 자료를 첨부해 이동시킬수는 없습니다.
        // 하지만 get 방식으로 이동하는 만큼 쿼리스트링을 붙여서 보내는것은 가능합니다.
        return "redirect:http://www.naver.com"; // return 은 .jsp파일만 가능함 하지만
                                                // 리다이렉트이기 때문에 네이버 주소같은곳으로 보낼수있음
    }

    @RequestMapping("/goFoodform")
    public String goFoodForm(Model model){
        // 리다이렉션은 model.addAttribute() 를 통한 데이터 전송이 불가능합니다.
        model.addAttribute("title", "음식주문 키오스크"); //  title라는 이름으로 "음식주문...." 문자열 적재
            // http://localhost:8080/chap03/foodform 이라는 주소로 이동(리다이렉트이므로 파일지정이 아님)
        return "redirect:chap03/foodform";
            // /WEB-INF/views/chap03/foodform.jsp 라는 목적 파일로 포워딩
        //return "chap03/foodform"; //목적 파일로 포워딩하지 않고 목적 메서드 주소로 리다이렉트
    }

    // 리다이렉트는 데이터 전달이 불가능한가요?
    // GET 방식은 url에 퀴리스트링을 붙여서 데이터를 전송하기 때문에
